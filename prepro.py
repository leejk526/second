...
def _process_caption_data(caption_file, image_dir, max_length):
    ...
    temp_id_to_filename = {}
    triple = math.ceil(len(id_to_filename) / 3)
    for key, value in id_to_filename.iteritems():
        temp_id_to_filename[key] = value
        if len(temp_id_to_filename.keys()) > triple:
            break
    id_to_filename = temp_id_to_filename

    # data is a list of dictionary which contains 'captions', 'file_name' and 'image_id' as key.
    data = []
    for annotation in caption_data['annotations']:
        image_id = annotation['image_id']
        if not image_id in id_to_filename:
            continue
