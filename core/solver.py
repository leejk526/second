...
class CaptioningSolver(object):
    def __init__(self, model, data, val_data, **kwargs):
        """
        Required Arguments:
            - model: SA&T caption generating model
            - data: Training data; dictionary with the following keys:
                - features: Feature vectors of shape (27595, 196, 512) # ceil(82783/3)
                - file_names: Image file names of shape (27595, )
                - captions: Captions of shape (150000, 17)
                - image_idxs: Indices for mapping caption to image of shape (150000, )
                - word_to_idx: Mapping directory from word to index
    ...
    def train(self):
        # train/val dataset
        n_examples = self.data['captions'].shape[0] # 27595
        ...
        # build graphs for training model and sampling cpations
        loss = self.model.build_model()
        with tf.variable_scope(tf.get_variable_scope()) as scope:
            with tf.name_scope('optimizer'):
                tf.get_variable_scope().reuse_variables()
                _, _, generated_captions = self.model.build_sampler(max_len=20) # time
            ...
            for e in range(self.n_epochs):
                ...
                for i in range(n_iters_per_epoch):
                    ...
                    if (i+1) % self.print_every == 0:
                        ...
                        for j, gt in enumerated(decoded):
